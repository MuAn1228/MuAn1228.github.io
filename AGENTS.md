...
- **顶部背景图（2026-08-24，提交 1b7f1e9）**：`source/img/finance/header-bg.webp`（由原 header-bg.png 264KB 经 sharp 转出 ~25KB），原 png 已删，CSS 引用 `/img/finance/header-bg.webp`。改 finance.css / index.md 后记得把 `<link ...finance.css?` 版本号 `?v=N` +1；改 finance-tracker.js 后把 `<script src="/js/finance-tracker.js?v=N">` 的 `?v=N` +1（当前 ?v=11），否则浏览器缓存旧代码。
...
- **Flappy Bird 地图 3「杭州夜航」+ 角色更换（2026-09-08，提交 f5f8ecc）**：三地图循环切换（`MAP_ORDER=['street','snow','hangzhou']`，localStorage 记忆）。杭州地图四段式旅程（`HZ_JOURNEY_LEN=4800`，hz.jx 仅 play 态前进）：0-25% 西湖（雷峰塔 h152 远景暖灯、保俶塔、垂柳、石拱桥、渔舟、荷叶）→ 25-50% 西湖城区（白墙黛瓦、路灯、灯笼招牌）→ 50-75% 钱塘江（斜拉桥桥面 y238、两岸灯火）→ 75-100% 钱江新城（高楼幕墙、杭州之门双子塔、LED 屏、高架 y318），旅程结束切 840px 无缝循环带（跨缝塔保证接缝）。4 层视差（sky/far 0.45x/mid 1.0x/ground+env FX），环境动画 hzEnvFX（星星闪烁/城市灯光 15 组/桥上车辆/高架车流/飞机/月影/波光）。障碍 6 种杭州化变体 64×52（0 石桥墩/1 雷峰塔身/2 白墙黛瓦/3 桥墩钢架/4 高楼幕墙/5 灯柱广告），按阶段映射 `HZ_STAGE_VARIANTS`。**角色改为用户参考图 `source/img/flappy/hero-sheet.png`（仅杭州地图使用）**：深灰底 rgb(40,43,48) 容差 13 紧阈值 flood-fill 抠帧（7 行 48 连通域），glide=滑翔行第 3 帧/flapUp=run[1]/flapDown=jump[2]/hurt=hurt[0]/idle=idle[2]，`BIRD_DRAW_H=46`；街景保留 body.png 静态贴纸少女，雪镇保留 penguin-sheet 企鹅帧（三张角色图始终独立加载，按 mapId 分配）。**动画帧面朝右（肤色像素判定），BIRD_FLIP=false 正确，勿改**。入场 1.4s「MAP 03 / 杭州夜航」标题。脚本版本 `?v=14`；改它必 bump。验证：`tmp_test/patch_hz.py`（17 处断言补丁可重放）+ `tmp_test/flappy-hz-probe.html`→`parse_hz_dom.py`（四阶段+两图回归+hero 帧导出，无头 Chrome `--dump-dom --virtual-time-budget=20000`）+ 真机浏览器三图实测 + jsdom 11 用例。
